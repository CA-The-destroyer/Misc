...
- Terminal Server / FSLogix safe
- Checks Chrome only in the CURRENT SESSION (ignores other users)
- Tries both Chrome paths: ProgramFiles(x86) first, then ProgramFiles
- Copies ONLY Bookmarks (+ .bak) from current user's active profile -> TargetProfile (default: Youtube_Test)
- Forgiving: if no source profile exists, skips copy and still launches
- Big RED banner if Chrome is running in this session
#>

param(
    [string]$TargetProfile    = "Youtube_Test",
    [string]$SourceProfile    = "",  # If empty, auto-detect from Local State (last_used), fallback to "Default"
...
if (-not $chrome) {
    Write-Error "Chrome not found in either standard location."
    exit 1
}
...
# --- Copy ONLY bookmarks (and .bak) if we have a valid, different source ---
if (-not $skipCopy) {
